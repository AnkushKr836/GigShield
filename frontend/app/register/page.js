...
export default function RegisterPage() {
  ...
  return (
    <div className="pt-2">
      ...
      {loadingOptions && <p className="text-sm text-storm-light mb-4">Loading…</p>}
      {noOptionsAvailable && (
        <p className="text-sm text-danger bg-danger/5 border border-danger/20 rounded-card px-3 py-2 mb-4">
          No zones or companies exist yet — seed them via the backend first.
        </p>
      )}
      {error && (
        <p className="text-sm text-danger bg-danger/5 border border-danger/20 rounded-card px-3 py-2 mb-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={submitting || zones.length === 0 || companies.length === 0}
...
function Field({ label, children }) {
  return (
    <label className="block">
      <span className="block text-xs font-medium text-storm-light mb-1.5 uppercase tracking-wide">{label}</span>
      {children}
    </label>
  );
}
